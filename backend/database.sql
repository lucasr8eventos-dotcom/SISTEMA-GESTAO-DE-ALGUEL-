...
CREATE DATABASE gestao_alugueis;
\c gestao_alugueis;
...
CREATE INDEX idx_pagamentos_imovel ON pagamentos(imovel_id);
CREATE INDEX idx_despesas_vencimento ON despesas(vencimento);
CREATE INDEX idx_despesas_status ON despesas(status);
CREATE INDEX idx_reajustes_data_proximo ON reajustes(data_proximo);
CREATE INDEX idx_reajustes_status ON reajustes(status);
